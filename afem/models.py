...
from .utils import default, batched_eye_like, batched_jacobian
...
class VectorSpinModel(nn.Module):
    ...
    def approximate_free_energy(self, phi, beta):
        beta = default(beta, self.beta)
        return - beta**-1 * (0.5*torch.log(math.pi / beta) + self.num_spins**-1 * phi)
    ...
    def forward(self, x, beta=None, return_responses=False, detach_responses=False):
        ...
        phi_star = self._phi(t_star, h, beta=beta)

        # Compute approximate free energy.
        afe = self.approximate_free_energy(phi_star, beta=beta)

        if return_responses:
            responses = batched_jacobian(afe, h, create_graph=not detach_responses)
            if detach_responses:
                responses = responses.detach()
            return afe, responses
        else:
            return afe
