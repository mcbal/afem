    ...
    with tqdm(train_loader, unit='it') as tqdm_loader:
        for idx, (data, target) in enumerate(tqdm_loader):
            ...
            tqdm_loader.set_postfix(
                loss_logp=f'{loss_logp.item():.2f}',
                loss_xent=f'{loss_xent.item():.2f}',
...
class CIFAR10Net(nn.Module):
    def __init__(self, dim=192, dim_conv=32, num_spins=16+16+1):
    ...
    def forward(self, xin, y=None):
        ...
        # breakpoint()
        # print(xin.shape)
        h = self.to_patch_embedding(xin)

        hlin = self.patch_linear(h)
        h = self.patch_act(hlin)

        # print(x.shape)

        cls_tokens = self.cls_token.repeat(h.shape[0], 1, 1)
        x = torch.cat((h, cls_tokens), dim=1)
        ...
        # kak = torch.linalg.slogdet(
        #     self.patch_linear.weight).logabsdet + (1 - torch.tanh(hlin).pow(2).log().sum(-1).mean(dim=-1).mean(dim=0))

        # print(torch.linalg.slogdet(
        #     self.patch_linear.weight).logabsdet, kak)
        # breakpoint()

        # print((1 - torch.tanh(hlin).pow(2).log().sum(-1)))
        # breakpoint()

        loss_logp = -out.log_prob.mean() / self.attention.spin_model.num_spins  # - kak
